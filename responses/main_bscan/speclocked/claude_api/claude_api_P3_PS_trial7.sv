...
module tap_controller (
    ...
    // IDCODE 32-bit shift register
    logic [31:0] idcode_shift_reg;

    // BYPASS 1-bit register
    logic        bypass_reg;
    ...
    //-----------------------------------------------------
    // IR shift register and hold register
    //-----------------------------------------------------
    always_ff @(posedge tck or negedge trst_n) begin
        if (!trst_n) begin
            ...
            if (current_state == CAPTURE_IR) begin
                // Fixed capture pattern: 01 in low bits per IEEE 1149.1
    ...
    //-----------------------------------------------------
    // BYPASS register (also used by CLAMP)
    //-----------------------------------------------------
    always_ff @(posedge tck or negedge trst_n) begin
        if (!trst_n) begin
            bypass_reg <= 1'b0;
    ...
    //-----------------------------------------------------
    // TDO output logic (negedge TCK launch)
    //-----------------------------------------------------
    logic tdo_data;
    ...
    always_comb begin
        if (current_state == SHIFT_IR) begin
            ...
            unique case (ir_hold_reg)
                INST_EXTEST,
                INST_SAMPLE:  tdo_data = bsr_tdo;
                INST_IDCODE:  tdo_data = idcode_shift_reg[0];
