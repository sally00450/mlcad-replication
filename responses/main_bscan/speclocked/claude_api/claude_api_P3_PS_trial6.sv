...
module tap_controller (
    ...
    // TDO output register (launched on negedge)
    logic tdo_neg;
    logic tdo_en_neg;
    ...
    always_ff @(posedge tck or negedge trst_n) begin
        if (!trst_n) begin
            ...
            case (current_state)
                ...
                CAPTURE_IR:       ir_shift_reg <= 4'b0001; // LSB=1 per 1149.1
    ...
    // IR hold register (updated on UPDATE_IR, reset to IDCODE)
    always_ff @(posedge tck or negedge trst_n) begin
        if (!trst_n)
            ir_hold_reg <= INST_IDCODE;
        else if (current_state == TEST_LOGIC_RESET)
            ir_hold_reg <= INST_IDCODE;
        else if (current_state == UPDATE_IR)
    ...
    always_ff @(posedge tck or negedge trst_n) begin
        ...
        end else if (current_state == CAPTURE_DR && ir_hold_reg == INST_IDCODE) begin
            idcode_shift_reg <= 32'hDEADBEEF;
    ...
    //------------------------------------------------------------------
    // BYPASS register (used for BYPASS, CLAMP, and default)
    //------------------------------------------------------------------
    always_ff @(posedge tck or negedge trst_n) begin
        if (!trst_n) begin
            bypass_reg <= 1'b0;
        end else if (current_state == CAPTURE_DR) begin
            bypass_reg <= 1'b0;
    ...
    always_comb begin
        ...
        if (current_state == SHIFT_IR) begin
            ...
            tdo_en_comb = 1'b1;
            case (ir_hold_reg)
                INST_EXTEST, INST_SAMPLE: tdo_mux = bsr_tdo;
                INST_IDCODE:              tdo_mux = idcode_shift_reg[0];
                INST_UTDR:                tdo_mux = tdr_tdo;
    ...
    always_ff @(negedge tck or negedge trst_n) begin
        if (!trst_n) begin
            ...
            tdo_neg    <= tdo_mux;
            tdo_en_neg <= tdo_en_comb;
        end
    end

    assign tdo    = tdo_neg;
    assign tdo_en = tdo_en_neg;
