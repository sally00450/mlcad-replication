#!/usr/bin/python3
# collect_minidut_nocontract_api.py -- Mini-DUT pilot, NO-PORT-CONTRACT variant.
# Adapted from collect_minidut_api.py. Collects n=3 cloud-hosted LLM API responses for
# PS_TAP with the port-contract block removed (baseline for R2 ablation).
import argparse, os, re, sys, json, time, datetime
import boto3

PROMPTS_MD = os.path.join(os.path.dirname(__file__), "..",
                          "prompts", "PROMPTS_MINIDUT_NOCONTRACT.md")
PROMPT_IDS = ["PS_TAP"]
...
def main():
    ...
    with open(os.path.join(out_root, "model_info.txt"), "w") as f:
        ...
        f.write("variant: NO_PORT_CONTRACT (R2 baseline)\n")

    start = time.time()
    for pid in PROMPT_IDS:
        prompt_text = extract_prompt(md, pid)
        for trial in TRIALS:
            # Write per-trial into responses_nocontract/trial{N}/
            trial_dir = os.path.join(os.path.dirname(out_root), "trial" + str(trial))
            os.makedirs(trial_dir, exist_ok=True)
            out_file = os.path.join(
                trial_dir, "claude_api_nocontract_" + pid + "_trial" + str(trial)
                + ".sv")
            # Also mirror in out_root for score script compatibility
            mirror_file = os.path.join(
                out_root, "claude_api_nocontract_" + pid + "_trial" + str(trial)
                + ".sv")
            if os.path.exists(out_file) and os.path.exists(mirror_file):
