...
PROMPTS_MD = "paper/web_ai_eval/PROMPTS.md"                                                                                                                                                                     
PROMPT_IDS = ["P1", "P2", "P3", "P4", "P5", "P5b", "P6"]                                                            
TRIALS = [1, 2, 3]                                                                                                                                                                                              
...
def main():                                                                                                                                                                                                     
    ...
    out_root = os.path.join("paper/web_ai_eval/responses", args.outdir)                                             
    ...
    for pid in PROMPT_IDS:                                                                                                                                                                                      
        ...
        for trial in TRIALS:                                                                                                                                                                                    
            ...
            out_file = os.path.join(out_root, prefix + "_" + pid + "_trial" + str(trial) + ".sv")                   
            if os.path.exists(out_file):                                                                                                                                                                        
                print("[{0}/{1}] SKIP exists: {2}".format(done, total, out_file))                                                                                                                               
                continue                                                                                                                                                                                        
            t0 = time.time()                                                                                                                                                                                    
            try:                                                                                                                                                                                                
            ...
            with open(out_file, "w") as f:                                                                          
                f.write(text)                                                                                                                                                                                   
            dt = time.time() - t0                                                                                                                                                                               
            print("[{0}/{1}] {2} trial {3}: {4} bytes in {5:.1f}s -> {6}".format(done, total, pid, trial, len(text), dt, out_file))                                                                             
