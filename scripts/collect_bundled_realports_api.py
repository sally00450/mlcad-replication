#!/usr/bin/python3
# collect_bundled_realports_api.py -- Collect 3 cloud-hosted LLM API responses for the
# real-port bundled prompt PB'' (all 4 modules in one SV file, with pad /
# bsc_cell / tap_controller port names pinned to the PS (VCS fixture)
# port contract rather than the Tessent pad-library attribute tags).
#
# Usage:
#   scripts/collect_bundled_realports_api.py \
#       --model <MODEL_ID_4_7> --outdir claude_api
#   scripts/collect_bundled_realports_api.py \
#       --model <MODEL_ID_4_6> --outdir claude46_api
#
# Responses are written to:
#   paper/web_ai_eval/responses/bundled_realports/<outdir>/
#       <outdir>_PBpp_trial{1,2,3}.sv
import argparse, os, re, sys, json, time, datetime
import boto3

PROMPTS_MD = "paper/web_ai_eval/PROMPTS_BUNDLED_REALPORTS.md"
PROMPT_ID_IN_MD = "PB''"
PROMPT_ID_OUT = "PBpp"  # used only in output filenames (apostrophes unsafe)
TRIALS = [1, 2, 3]
...
def main():
    ...
    ap.add_argument("--outdir", required=True,
                    help="subdir under responses/bundled_realports/ "
                         "and filename prefix")
    ...
    out_root = os.path.join(
        "paper/web_ai_eval/responses/bundled_realports", args.outdir)
    ...
    for trial in TRIALS:
        ...
        with open(out_file, "w") as f:
            f.write(text)
        dt = time.time() - t0
        print("[{0}/{1}] PBpp trial {2}: {3} bytes in {4:.1f}s -> {5}"
